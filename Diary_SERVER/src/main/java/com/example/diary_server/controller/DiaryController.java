...
@Tag(name = "Diary 만들기", description = "나만의 감정일기장 만들기")
public class DiaryController {
    private final DiaryService diaryService;

    //diary에 추가
    @PostMapping
    @Operation(summary = "다이어리에 추가", description = "content,emotionId,date를 받아 저장")
    ...
    //날짜로 찾기
    @GetMapping
    @Operation(summary = "월별로 찾기", description = "검색하고 싶은 달을 입력받아 해당하는 달에 저장된 모든 다이어리를 가져옴")
    ...
    //id로 찾기
    @GetMapping("/{id}")
    @Operation(summary = "id로 조회", description = "특정 id를 받아 다이어리 조회")
    ...
    @PutMapping("/{id}")
    @Operation(summary = "수정하기", description = "일기 수정, id 를 받아서 새로 받은 내용으로 덮어쓰기")
    ...
    //id로 삭제하기
    @DeleteMapping("/{id}")
    @Operation(summary = "id로 삭제", description = "id를 받아 다이어리에서 삭제")
